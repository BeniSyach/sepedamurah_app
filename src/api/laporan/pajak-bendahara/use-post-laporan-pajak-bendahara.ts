...
import { type LaporanPajakBendahara } from './types'

interface CreateLaporanPajakBendaharaPayload {
  user_id: string
  kd_opd1: string
  kd_opd2: string
  kd_opd3: string
 ...
 */
export function usePostLaporanPajakBendahara() {
  const queryClient = useQueryClient()

  return useMutation({
    mutationFn: async (
      payload: CreateLaporanPajakBendaharaPayload | FormData
    ) => {
      const { data } = await api.post<LaporanPajakBendahara>(
