...
import { usePutLaporanSp2bToBUD, type LaporanSp2bToBUD } from '@/api'
...
type LaporanSp2bToBUDPeriksaProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  currentRow: LaporanSp2bToBUD
...
type LaporanSp2bToBUDForm = z.infer<typeof formSchema>

export function LaporanSp2bToBUDPeriksa({
  open,
  onOpenChange,
  currentRow,
  onAction,
}: LaporanSp2bToBUDPeriksaProps) {
  const [fileUrl, setFileUrl] = useState<string | null>(null)
  const user = useAuthStore((s) => s.user)

  const { mutateAsync: putLaporanSp2bToBUDAsync } = usePutLaporanSp2bToBUD()

  const form = useForm<LaporanSp2bToBUDForm>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      id: currentRow?.id.toString() || '',
      nama_file: currentRow?.refSp2bKeBud?.nm_sp2b_ke_bud || '',
  ...
  const handleAction = async (action: 'terima' | 'tolak') => {
    ...
    try {
      ...
      // Kirim ke backend
      await toast.promise(putLaporanSp2bToBUDAsync(formData), {
                  ...
                  <Form {...form}>
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          value={
                            currentRow?.refSp2bKeBud?.nm_sp2b_ke_bud || '-'
                          }
                          readOnly
                        />
                      </FormControl>
