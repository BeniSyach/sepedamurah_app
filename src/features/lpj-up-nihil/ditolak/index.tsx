...
const route = getRouteApi('/_authenticated/dokumen/lpj-up-nihil/ditolak')

// Hitung awal & akhir bulan sekarang
const defaultFrom = startOfMonth(new Date())
const defaultTo = endOfMonth(new Date())

export function PermohonanDitolakLPJUPNihil() {
  ...
  const params = {
    ...
    menu:
      userRole === 'Operator SKPKD'
        ? 'permohonan_lpj_up_nihil_tolak_operator'
        : 'lpj_up_nihil_ditolak',
    // ...(shouldSendDate && {
          ...
          <div className='mb-2 flex flex-wrap items-center justify-between space-y-2'>
            <div>
              <h2 className='text-2xl font-bold tracking-tight'>
                Permohonan Ditolak LPJ UPNIHIL
              </h2>
              <p className='text-muted-foreground'>
                Data Ini adalah Permohonan Ditolak LPJ UPNIHIL
              </p>
            </div>
          </div>
