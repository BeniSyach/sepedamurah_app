...
type LaporanRekonsiliasiGajiSKPDPeriksaProps = {
  open: boolean
...
export function LaporanRekonsiliasiGajiSKPDPeriksa({
  open,
  onOpenChange,
  currentRow,
  onAction,
}: LaporanRekonsiliasiGajiSKPDPeriksaProps) {
  const [fileUrl, setFileUrl] = useState<string | null>(null)
  const user = useAuthStore((s) => s.user)

  const { mutateAsync: putLaporanRekonsiliasiGajiSKPDAsync } =
    usePutLaporanRekonsiliasiGajiSKPD()
  ...
  const handleAction = async (action: 'terima' | 'tolak') => {
    ...
    try {
      ...
      // Kirim ke backend
      await toast.promise(putLaporanRekonsiliasiGajiSKPDAsync(formData), {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContentLarge
        title='Periksa Berkas Laporan Rekonsiliasi Gaji SKPD'
              ...
              <Form {...form}>
                <div className='space-y-4'>
                  <Form {...form}>
                    {/* NAMA FILE */}
                    <FormItem>
                      <FormLabel>Nama Rekonsiliasi Gaji SKPD</FormLabel>
                      <FormControl>
                        <Input
                          value={
                            currentRow?.RekonsiliasiGajiSKPD
