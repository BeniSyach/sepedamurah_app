...
import {
  ...
  usePostLaporanRekonsiliasiGajiSKPD,
  usePutLaporanRekonsiliasiGajiSKPD,
  type LaporanRekonsiliasiGajiSKPD,
} from '@/api'
...
type LaporanRekonsiliasiGajiSKPDForm = z.infer<typeof formSchema>

type Props = {
  currentRow?: LaporanRekonsiliasiGajiSKPD
  open: boolean
...
export function LaporanRekonsiliasiGajiSKPDActionDialog({
  currentRow,
  open,
  onOpenChange,
}: Props) {
  ...
  /* API */
  const { mutateAsync: postAsync } = usePostLaporanRekonsiliasiGajiSKPD()
  const { mutateAsync: putAsync } = usePutLaporanRekonsiliasiGajiSKPD()
  ...
  /* Form */
  const form = useForm<LaporanRekonsiliasiGajiSKPDForm>({
    resolver: zodResolver(formSchema),
    defaultValues: isEdit
      ? {
          id: currentRow.id.toString(),
          tahun: currentRow.tahun,
          rek_gaji_id: currentRow.rek_gaji_id?.toString(),
  ...
  /* Submit Handler */
  const onSubmit = async (data: LaporanRekonsiliasiGajiSKPDForm) => {
    ...
    >
      <DialogContent className='sm:max-w-lg'>
        <DialogHeader className='text-start'>
          <DialogTitle>
            {isEdit
              ? 'Edit Laporan Rekonsiliasi Gaji SKPD'
              : 'Tambah Laporan Rekonsiliasi Gaji SKPD'}
          </DialogTitle>
          <DialogDescription>
            {isEdit
              ? 'Perbarui data Laporan Rekonsiliasi Gaji SKPD.'
              : 'Tambahkan Laporan Rekonsiliasi Gaji SKPD baru.'}
          </DialogDescription>
        </DialogHeader>
                            ...
                            <CommandGroup>
                              <CommandList>
                                {listDPA.map((d) => (
                                  <CommandItem
                                    key={d.id}
                                    value={d.nm_rekonsiliasi_gaji_skpd}
                                    onSelect={() =>
                                      form.setValue('rek_gaji_id', String(d.id))
                                    }
                                  >
                                    <CheckIcon
                                      className={cn(
