...
import { LaporanRekonsiliasiGajiSKPDProvider } from './components/laporan-dpa-diterima-provider'
import { LaporanRekonsiliasiGajiSKPDDiterimaTable } from './components/laporan-dpa-diterima-table'

const route = getRouteApi(
  '/_authenticated/dokumen/laporan-rekonsiliasi-gaji-skpd/laporan-rekonsiliasi-gaji-skpd-diterima'
)

export function LaporanRekGajiSKPDDiterima() {
  ...
  const { data, isLoading, isError } = useGetLaporanRekonsiliasiGajiSKPD({
    ...
    menu:
      userRole === 'Operator SKPKD'
        ? 'operator_laporan_rek_gaji_diterima'
        : 'laporan_rek_gaji_diterima',
      ...
      <Main>
        <div className='mb-2 flex flex-wrap items-center justify-between space-y-2'>
          <div>
            <h2 className='text-2xl font-bold tracking-tight'>
              Laporan Rekonsiliasi Gaji SKPD Diterima
            </h2>
            <p className='text-muted-foreground'>
              Data Ini adalah Laporan Rekonsiliasi Gaji SKPD Diterima
            </p>
          </div>
        </div>
        <div className='-mx-4 flex-1 overflow-auto px-4 py-1 lg:flex-row lg:space-y-0 lg:space-x-12'>
          ...
          {isLoading ? (
            ...
            <LaporanRekonsiliasiGajiSKPDDiterimaTable
              data={data?.data ?? []}
              meta={data?.meta}
              search={search}
              navigate={navigate}
