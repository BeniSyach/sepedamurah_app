...
import { usePutTerimaRekonsiliasiGajiSKPDMulti } from '@/api'
...
export function TerimaFungsionalMultiDialog<TData>({
  ...
  const { mutateAsync: terimaMulti } = usePutTerimaRekonsiliasiGajiSKPDMulti()
  ...
  const handleTerima = () => {
    ...
    toast.promise(
      ...
      {
        loading: 'Mengirim data Laporan Rekonsiliasi Gaji SKPD...',
        success: () => {
          table.resetRowSelection()
          return `Berhasil menerima ${selectedRows.length} Laporan Rekonsiliasi Gaji SKPD.`
        },
        error: (err) => {
      ...
      title={
        <span>
          ...
          {selectedRows.length > 1
            ? 'Berkas Masuk Rekonsiliasi Gaji SKPD'
            : 'Berkas Masuk Rekonsiliasi Gaji SKPD'}
        </span>
      }
      desc={
        <div className='space-y-4'>
          <p className='mb-2'>
            Apakah Anda yakin Terima Berkas Masuk Rekonsiliasi Gaji SKPD ini?{' '}
            <br />
