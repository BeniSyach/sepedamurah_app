...
import { LaporanRekonsiliasiGajiSKPDProvider } from './components/laporan-dpa-ditolak-provider'
import { LaporanRekonsiliasiGajiSKPDDitolakTable } from './components/laporan-dpa-ditolak-table'

const route = getRouteApi(
  '/_authenticated/dokumen/laporan-rekonsiliasi-gaji-skpd/laporan-rekonsiliasi-gaji-skpd-ditolak'
)

export function LaporanRekGajiSKPDDitolak() {
  ...
  const { data, isLoading, isError } = useGetLaporanRekonsiliasiGajiSKPD({
    ...
    menu:
      userRole === 'Operator SKPKD'
        ? 'operator_laporan_rek_gaji_ditolak'
        : 'laporan_rek_gaji_ditolak',
      ...
      <Main>
        <div className='mb-2 flex flex-wrap items-center justify-between space-y-2'>
          <div>
            <h2 className='text-2xl font-bold tracking-tight'>
              Laporan Rekonsiliasi Gaji SKPD Ditolak
            </h2>
            <p className='text-muted-foreground'>
              Data Ini adalah Laporan Rekonsiliasi Gaji SKPD Ditolak
            </p>
          </div>
        </div>
        <div className='-mx-4 flex-1 overflow-auto px-4 py-1 lg:flex-row lg:space-y-0 lg:space-x-12'>
          ...
          {isLoading ? (
            ...
            <LaporanRekonsiliasiGajiSKPDDitolakTable
              data={data?.data ?? []}
              meta={data?.meta}
              search={search}
              navigate={navigate}
