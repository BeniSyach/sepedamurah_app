...
type SKPDMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function SKPDsMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: SKPDMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = () => {
    ...
    toast.promise(sleep(2000), {
      loading: 'Menghapus SKPD...',
      success: () => {
        table.resetRowSelection()
        return `Telah Dihapus ${selectedRows.length} ${
          selectedRows.length > 1 ? 'SKPDs' : 'SKPD'
        }`
      },
      error: 'Error',
    })
