...
type JenisBelanjaMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function JenisBelanjasMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: JenisBelanjaMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = () => {
    ...
    toast.promise(sleep(2000), {
      loading: 'Menghapus Jenis Belanja...',
      success: () => {
        table.resetRowSelection()
        return `Telah Dihapus ${selectedRows.length} ${
          selectedRows.length > 1 ? 'Jenis Belanjas' : 'Jenis Belanja'
        }`
      },
      error: 'Error',
    })
