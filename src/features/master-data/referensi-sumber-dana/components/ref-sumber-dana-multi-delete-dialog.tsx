...
type SumberDanaMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function SumberDanasMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: SumberDanaMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = () => {
    ...
    toast.promise(sleep(2000), {
      loading: 'Menghapus Sumber Dana...',
      success: () => {
        table.resetRowSelection()
        return `Telah Dihapus ${selectedRows.length} ${
          selectedRows.length > 1 ? 'Sumber Danas' : 'Sumber Dana'
        }`
      },
      error: 'Error',
    })
