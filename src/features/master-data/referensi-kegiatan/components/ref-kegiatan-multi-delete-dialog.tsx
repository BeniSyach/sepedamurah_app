...
type KegiatanMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function KegiatansMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: KegiatanMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = () => {
    ...
    toast.promise(sleep(2000), {
      loading: 'Menghapus Kegiatans...',
      success: () => {
        table.resetRowSelection()
        return `Telah Dihapus ${selectedRows.length} ${
          selectedRows.length > 1 ? 'Kegiatans' : 'Kegiatan'
        }`
      },
      error: 'Error',
    })
