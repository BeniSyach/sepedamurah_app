...
type SubKegiatanMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function SubKegiatansMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: SubKegiatanMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = () => {
    ...
    toast.promise(sleep(2000), {
      loading: 'Menghapus Sub Kegiatan...',
      success: () => {
        table.resetRowSelection()
        return `Telah Dihapus ${selectedRows.length} ${
          selectedRows.length > 1 ? 'Sub Kegiatans' : 'Sub Kegiatan'
        }`
      },
      error: 'Error',
    })
