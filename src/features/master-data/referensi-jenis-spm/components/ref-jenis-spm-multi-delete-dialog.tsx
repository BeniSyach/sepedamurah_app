...
type JenisSPMMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function JenisSPMsMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: JenisSPMMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = () => {
    ...
    toast.promise(sleep(2000), {
      loading: 'Menghapus Jenis SPM...',
      success: () => {
        table.resetRowSelection()
        return `Telah Dihapus ${selectedRows.length} ${
          selectedRows.length > 1 ? 'JenisSPMs' : 'JenisSPM'
        }`
      },
      error: 'Error',
    })
