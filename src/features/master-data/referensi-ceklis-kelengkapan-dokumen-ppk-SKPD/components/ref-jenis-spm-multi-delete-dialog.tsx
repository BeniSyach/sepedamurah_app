...
type CeklisSPMMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function CeklisSPMsMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: CeklisSPMMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = () => {
    ...
    toast.promise(sleep(2000), {
      loading: 'Menghapus Ceklis Kelengkapan Dokumen PPK SKPD...',
      success: () => {
        table.resetRowSelection()
        return `Telah Dihapus ${selectedRows.length} ${
          selectedRows.length > 1
            ? 'Ceklis Kelengkapan Dokumen PPK SKPD'
            : 'Ceklis Kelengkapan Dokumen PPK SKPD'
        }`
      },
      error: 'Error',
