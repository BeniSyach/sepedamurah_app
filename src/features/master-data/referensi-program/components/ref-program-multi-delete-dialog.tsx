...
type ProgramMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function ProgramsMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: ProgramMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = () => {
    ...
    toast.promise(sleep(2000), {
      loading: 'Menghapus Programs...',
      success: () => {
        table.resetRowSelection()
        return `Telah Dihapus ${selectedRows.length} ${
          selectedRows.length > 1 ? 'Programs' : 'Program'
        }`
      },
      error: 'Error',
    })
