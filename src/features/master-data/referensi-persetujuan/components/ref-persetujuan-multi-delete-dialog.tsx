...
type PersetujuanMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function PersetujuansMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: PersetujuanMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = () => {
    ...
    toast.promise(sleep(2000), {
      loading: 'Menghapus Persetujuan...',
      success: () => {
        table.resetRowSelection()
        return `Telah Dihapus ${selectedRows.length} ${
          selectedRows.length > 1 ? 'Persetujuans' : 'Persetujuan'
        }`
      },
      error: 'Error',
    })
