...
type TanggalForm = z.infer<typeof formSchema>

type TanggalDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function PengembalianRekapPdf({
  open,
  onOpenChange,
}: TanggalDialogProps) {
  const form = useForm<TanggalForm>({
  ...
  const onSubmit = async (values: TanggalForm) => {
    await toast.promise(
      (async () => {
        ...
        link.setAttribute(
          'download',
          `Pengembalian-${tanggal_awal}-${tanggal_akhir}.pdf`
        )
    ...
    >
      <DialogContent className='sm:max-w-sm'>
        <DialogHeader className='text-start'>
          ...
          <DialogDescription>
            Pilih tanggal awal dan akhir untuk melanjutkan.
          </DialogDescription>
        </DialogHeader>
