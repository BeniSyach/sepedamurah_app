...
import { usePutLaporanPajakBendahara, type LaporanPajakBendahara } from '@/api'
...
type LaporanPajakBendaharaPeriksaProps = {
  open: boolean
...
export function PenerimaanPeriksa({
  ...
  currentRow,
  onAction,
}: LaporanPajakBendaharaPeriksaProps) {
  const [fileUrl, setFileUrl] = useState<string | null>(null)
  const user = useAuthStore((s) => s.user)

  const { mutateAsync: putLaporanPajakBendaharaAsync } =
  ...
  const form = useForm<LaporanPajakBendaharaForm>({
    ...
    defaultValues: {
      ...
      nama_file: currentRow?.refPajakBendahara?.nm_pajak_bendahara || '',
  ...
  const handleAction = async (action: 'terima' | 'tolak') => {
    ...
    try {
      ...
      // Kirim ke backend
      await toast.promise(putLaporanPajakBendaharaAsync(formData), {
              ...
              <Form {...form}>
                <div className='space-y-4'>
                  <Form {...form}>
                    {/* NAMA FILE */}
                    <FormItem>
                      <FormLabel>Nama Laporan Pajak Bendara</FormLabel>
                      <FormControl>
                        <Input
                          value={
