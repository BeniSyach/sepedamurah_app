...
import { LaporanPajakBendaharaActionDialog } from './laporan-pajak-bendahara-action-dialog'
import { LaporanPajakBendaharaPeriksa } from './laporan-pajak-bendahara-periksa'
import { useRefLaporanPajakBendahara } from './laporan-pajak-bendahara-provider'
...
export function UsersDialogs() {
  ...
  const handleDelete = async () => {
    ...
    await toast.promise(deletePromise, {
      ...
      success: () => {
        ...
        return `Data "${currentRow.refPajakBendahara?.nm_pajak_bendahara}" berhasil dihapus.`
      },
      error: (err) => {
  ...
  return (
    <>
      <LaporanPajakBendaharaActionDialog
        key='laporan-laporan-pajak-bendahara-penerimaan-add'
        open={open === 'add'}
      ...
      {currentRow && (
        <>
          <LaporanPajakBendaharaActionDialog
            key={`laporan-laporan-pajak-bendahara-penerimaan-edit-${currentRow.id}`}
            open={open === 'edit'}
            onOpenChange={() => {
              setOpen('edit')
              setTimeout(() => {
          ...
          <LaporanPajakBendaharaPeriksa
            key={`laporan-laporan-pajak-bendahara-penerimaan-periksa-${currentRow.id}`}
            open={open === 'periksa'}
            onOpenChange={() => {
              setOpen('periksa')
              setTimeout(() => {
          ...
          <ConfirmDialog
            key={`laporan-laporan-pajak-bendahara-penerimaan-delete-${currentRow.id}`}
            destructive
            open={open === 'delete'}
            onOpenChange={() => {
              setOpen('delete')
            ...
            className='max-w-md'
            title={`Hapus Laporan pajak bendahara Ini: ${currentRow.id} ?`}
            desc={
              <>
          ...
          <ConfirmDialog
            key={`laporan-pajak-bendahara-lihat-${currentRow.id}`}
            destructive={false}
            open={open === 'lihat'}
            onOpenChange={() => {
              setOpen('lihat')
