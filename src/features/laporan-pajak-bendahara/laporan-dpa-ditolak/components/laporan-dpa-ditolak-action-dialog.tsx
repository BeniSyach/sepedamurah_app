...
import {
  ...
  usePostLaporanPajakBendahara,
  usePutLaporanPajakBendahara,
  type LaporanPajakBendahara,
} from '@/api'
...
const formSchema = z.object({
  ...
  ref_pajak_id: z.string().min(1, 'pajak Bendahara harus dipilih.'),
...
type LaporanPajakBendaharaForm = z.infer<typeof formSchema>

type LaporanPajakBendaharaActionDialogProps = {
  currentRow?: LaporanPajakBendahara
  open: boolean
...
export function UsersActionDialog({
  ...
  open,
  onOpenChange,
}: LaporanPajakBendaharaActionDialogProps) {
  ...
  /* API */
  const { mutateAsync: postAsync } = usePostLaporanPajakBendahara()
  const { mutateAsync: putAsync } = usePutLaporanPajakBendahara()
  const { data: dataDPA } = useGetRefPajakBendahara({ page: 1, perPage: 9999 })

  /* Form */
  const form = useForm<LaporanPajakBendaharaForm>({
    resolver: zodResolver(formSchema),
    defaultValues: isEdit
      ? {
          id: currentRow.id.toString(),
          tahun: currentRow.tahun,
          ref_pajak_id: currentRow.ref_pajak_id?.toString(),
  ...
  /* Submit Handler */
  const onSubmit = async (data: LaporanPajakBendaharaForm) => {
    ...
    >
      <DialogContent className='sm:max-w-lg'>
        <DialogHeader className='text-start'>
          <DialogTitle>
            {isEdit ? 'Edit Pajak Bendahara' : 'Tambah Pajak Bendahara'}
          </DialogTitle>
          <DialogDescription>
            {isEdit
              ? 'Perbarui data Pajak Bendahara.'
              : 'Tambahkan Pajak Bendahara baru.'}
          </DialogDescription>
        </DialogHeader>
