...
import {
  type MasterSkpd,
  useGetRefSKPD,
  usePostAksesSp2bToBUD,
  usePutAksesSp2bToBUD,
  type AksesSp2bToBUDGroup,
  useGetRefSp2bToBUD,
} from '@/api'
...
type AksesDPASKPDActionDialogProps = {
  currentRow?: AksesSp2bToBUDGroup
  open: boolean
...
export function AksesDPASKPDActionDialog({
  ...
  const { mutateAsync: post } = usePostAksesSp2bToBUD()
  const { mutateAsync: put } = usePutAksesSp2bToBUD()
  ...
  const { data: dataDPA } = useGetRefSp2bToBUD({
    page: 1,
  ...
  const form = useForm<AksesDPASKPDForm>({
    ...
    defaultValues: currentRow
      ? {
          ...
          sp2bIds: currentRow?.sp2b?.map((d) => String(d.id)) ?? [],
        }
      : {
