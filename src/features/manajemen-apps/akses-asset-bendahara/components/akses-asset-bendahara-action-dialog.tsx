...
import {
  type MasterSkpd,
  useGetRefSKPD,
  usePostAksesAssetBendahara,
  usePutAksesAssetBendahara,
  type AksesAssetBendaharaGroup,
...
type AksesDPASKPDActionDialogProps = {
  currentRow?: AksesAssetBendaharaGroup
  open: boolean
...
export function AksesDPASKPDActionDialog({
  ...
  const { mutateAsync: post } = usePostAksesAssetBendahara()
  const { mutateAsync: put } = usePutAksesAssetBendahara()
  ...
  const { data: dataDPA } = useGetRefAssetBendahara({
    page: 1,
  ...
  const form = useForm<AksesDPASKPDForm>({
    ...
    defaultValues: currentRow
      ? {
          ...
          assetIds: currentRow?.asset?.map((d) => String(d.id)) ?? [],
        }
      : {
                              ...
                              <CommandGroup>
                                {listDPA.map((item) => {
                                  ...
                                  return (
                                    ...
                                    >
                                      <span>{item.nm_asset_bendahara}</span>
