...
type UserRoleMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function UserRolesMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: UserRoleMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = () => {
    ...
    toast.promise(sleep(2000), {
      loading: 'Menghapus User Role...',
      success: () => {
        table.resetRowSelection()
        return `Telah Dihapus ${selectedRows.length} ${
          selectedRows.length > 1 ? 'user Roles' : 'user Role'
        }`
      },
      error: 'Error',
    })
