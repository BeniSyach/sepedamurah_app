...
import {
  type MasterSkpd,
  useGetRefSKPD,
  usePostAksesRekonsiliasiGajiSkpd,
  usePutAksesRekonsiliasiGajiSkpd,
  type AksesRefRekonsiliasiGajiSkpdGroup,
  useGetRefRekonsiliasiGajiSkpd,
} from '@/api'
...
type AksesRekonsiliasiGajiSkpdForm = z.infer<typeof formSchema>

type AksesRekonsiliasiGajiSkpdActionDialogProps = {
  currentRow?: AksesRefRekonsiliasiGajiSkpdGroup
  open: boolean
...
export function AksesRekonsiliasiGajiSkpdActionDialog({
  currentRow,
  open,
  onOpenChange,
}: AksesRekonsiliasiGajiSkpdActionDialogProps) {
  const isEdit = !!currentRow

  const { mutateAsync: post } = usePostAksesRekonsiliasiGajiSkpd()
  const { mutateAsync: put } = usePutAksesRekonsiliasiGajiSkpd()
  ...
  const { data: dataDPA } = useGetRefRekonsiliasiGajiSkpd({
    page: 1,
    perPage: 9999, // biar ambil semua DPA
  })

  const form = useForm<AksesRekonsiliasiGajiSkpdForm>({
    ...
    defaultValues: currentRow
      ? {
          ...
          rekGajiIds: currentRow?.rekonsiliasi?.map((d) => String(d.id)) ?? [],
        }
      : {
  ...
  const onSubmit = async (data: AksesRekonsiliasiGajiSkpdForm) => {
    const payload = {
    ...
    await toast.promise(requestPromise, {
      ...
      success: () => {
        ...
        form.reset()
        return isEdit
          ? 'Data Rekonsiliasi Gaji SKPD berhasil diperbarui!'
          : 'Data Rekonsiliasi Gaji SKPD berhasil ditambahkan!'
      },
      error: (err) => {
        const message =
      ...
      <DialogContent className='sm:max-w-lg'>
        <DialogHeader className='text-start'>
          <DialogTitle>
            {' '}
            {isEdit
              ? 'Edit Rekonsiliasi Gaji SKPD'
              : 'Tambah Rekonsiliasi Gaji SKPD'}
          </DialogTitle>
          <DialogDescription>
            {isEdit
              ? 'Perbarui Rekonsiliasi Gaji SKPD disini. '
              : 'Tambah baru Rekonsiliasi Gaji SKPD disini. '}
        ...
        <div className='h-[26.25rem] w-[calc(100%+0.75rem)] overflow-y-auto py-1 pe-3'>
          <Form {...form}>
            <form
              id='akses-Asset-bendahara-skpd-form'
                      ...
                      <div className='col-span-4 w-full'>
                        <FormControl>
                          <Command className='rounded-md border'>
                            <CommandInput placeholder='Cari Rekonsiliasi Gaji SKPD...' />
        ...
        <DialogFooter>
          <Button type='submit' form='akses-Asset-bendahara-skpd-form'>
            Simpan Perubahan
          </Button>
        </DialogFooter>
      </DialogContent>
