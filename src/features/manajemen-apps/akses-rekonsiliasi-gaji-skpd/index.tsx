import { format } from 'date-fns'
import { getRouteApi } from '@tanstack/react-router'
import { useGetAksesRekonsiliasiGajiSkpd } from '@/api'
import { ConfigDrawer } from '@/components/config-drawer'
import { Header } from '@/components/layout/header'
import { Main } from '@/components/layout/main'
import { ProfileDropdown } from '@/components/profile-dropdown'
import { Search } from '@/components/search'
import { ThemeSwitch } from '@/components/theme-switch'
import { RefRekeningPrimaryButtons } from './components/akses-sp2b-to-bud-buttons'
import { UsersDialogs } from './components/akses-sp2b-to-bud-dialogs'
import { AksesRefRekonsiliasiGajiSkpdGroupProvider } from './components/akses-sp2b-to-bud-provider'
import { AksesSp2bToBUDTable } from './components/akses-sp2b-to-bud-table'

const route = getRouteApi(
  '/_authenticated/manajemen-apps/akses-rekonsiliasi-gaji-skpd'
)

export function AksesRekGajiSKPD() {
  const search = route.useSearch()
  const navigate = route.useNavigate()
  const tahunFilter = search.tahun ?? format(new Date(), 'yyyy')

  // 🔥 Ambil data langsung dari Laravel API
  const { data, isLoading, isError } = useGetAksesRekonsiliasiGajiSkpd({
    page: search.page,
    perPage: search.pageSize,
    search: search.search,
    tahun: tahunFilter,
  })

  return (
    <AksesRefRekonsiliasiGajiSkpdGroupProvider>
      <Header fixed>
        <Search />
        <div className='ms-auto flex items-center space-x-4'>
          <ThemeSwitch />
          <ConfigDrawer />
          <ProfileDropdown />
        </div>
      </Header>

      <Main>
        <div className='mb-2 flex flex-wrap items-center justify-between space-y-2'>
          <div>
            <h2 className='text-2xl font-bold tracking-tight'>
              Akses Rekonsiliasi Gaji SKPD
            </h2>
            <p className='text-muted-foreground'>
              Data Ini adalah Akses Rekonsiliasi Gaji SKPD
            </p>
          </div>
          <RefRekeningPrimaryButtons />
        </div>
        <div className='-mx-4 flex-1 overflow-auto px-4 py-1 lg:flex-row lg:space-y-0 lg:space-x-12'>
          {/* <UsersTable data={users} search={search} navigate={navigate} /> */}
          {isLoading ? (
            <p>Loading...</p>
          ) : isError ? (
            <p>Failed to load users.</p>
          ) : (
            <AksesSp2bToBUDTable
              data={data?.data ?? []}
              meta={data?.meta}
              search={search}
              navigate={navigate}
            />
          )}
        </div>
      </Main>

      <UsersDialogs />
    </AksesRefRekonsiliasiGajiSkpdGroupProvider>
  )
}
