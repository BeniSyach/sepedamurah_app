...
type BidangUrusanMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function BidangUrusanMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: BidangUrusanMultiDeleteDialogProps<TData>) {
  ...
  const handleDelete = () => {
    ...
    toast.promise(sleep(2000), {
      loading: 'Menghapus Bidang Urusan...',
      success: () => {
        table.resetRowSelection()
        return `Telah Dihapus ${selectedRows.length} ${
          selectedRows.length > 1 ? 'Bidang Urusan' : 'Bidang Urusan'
        }`
      },
      error: 'Error',
    })
